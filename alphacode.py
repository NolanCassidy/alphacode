...
def alphacode(pin):
    ...
    mnemonic = ""
    if  len(str(pin)) == 1:
        print("Please enter at least two digits.")
    elif len(str(pin)) % 2 == 1:
        print("Please enter at even amount of digits.")
    elif len(str(pin)) > 1:
        while pin > 0:
            hold = pin % 100
            mnemonic = VOWELS[hold % 5] +  mnemonic
            mnemonic = CONSONANTS[hold // 5] + mnemonic
            pin = pin // 100
...
def main():
    ...
    if len(str(pin)) > 1 and len(str(pin)) % 2 == 0:
